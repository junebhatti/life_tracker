...
<html>
  <body style="font-family: system-ui, sans-serif; max-width: 640px; margin: 80px auto; color: #111; line-height: 1.5;">
    <h1 style="font-size: 18px;">Google Health</h1>
    <p>${message}</p>
    ${
      token
...
/** Exchanges the one-time auth code for a refresh token to store as GOOGLE_HEALTH_REFRESH_TOKEN. */
export async function GET(request: NextRequest) {
  ...
  return htmlResponse(
    "Connected. Copy this value into the <strong>GOOGLE_HEALTH_REFRESH_TOKEN</strong> environment variable " +
      "in your Vercel project settings, then redeploy. Make sure the OAuth consent screen's publishing status is " +
      "<strong>In production</strong> (not Testing) — Testing-mode refresh tokens expire after 7 days.",
